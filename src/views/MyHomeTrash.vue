<template>
  <div class="trash-container">
    <!-- <div class="section-title">垃圾箱</div> -->
    ...
    <div v-loading="loading" element-loading-text="加载中...">
      <div v-if="trashList.length > 0" class="article-list">
        <div v-for="article in trashList" :key="article.id" class="article-item">
              ...
              <div class="title-row">
                ...
                <div class="article-actions">
                  <el-button 
                    ...
                    size="small" 
                    link
                    @click.stop="recoverArticle(article.id)"
                  >
      ...
      <el-empty v-else description="垃圾箱是空的" />
    </div>
    
    <!-- 分页组件 -->
    <div class="pagination-container" v-if="trashList.length > 0">
      <el-pagination
...
const loadTrashList = async () => {
  ...
  try {
    ...
    if (data && data.data && data.data.list) {
      trashList.value = data.data.list
      // 保存总记录数
      total.value = data.data.total || 0
    }
  } catch (error) {
    console.error('获取回收站列表失败:', error)
    ElMessage.error('获取回收站列表失败，请稍后重试')
  } finally {
    loading.value = false
  }
}
