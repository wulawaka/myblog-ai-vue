...
- **接口名称**: 更新标签名称
- **请求方法**: PUT
- **请求路径**: `/api/category/change-tag-name`
- **功能描述**: 更新指定标签的名称（需要登录权限）
...
|--------|------|------|--------|------|
| id | Long | 是 | `1` | 标签 ID |
| name | String | 是 | `"Java 编程"` | 标签新名称，1-6 位字符 |
...
| data | Object | 响应数据 |
| data.id | Long | 标签 ID |
| data.name | String | 更新后的标签名称 |
| data.parentId | Long | 父标签 ID |
| data.updatedAt | DateTime | 更新时间戳 |
...
- **请求方法**: PUT
- **请求路径**: `/api/article`
- **功能描述**: 更新已有文章的信息，支持修改主标签和子标签（需要登录权限）
...
| categoryId | Long | 是 | `1` | 分类 ID，必须是主标签（parent_id=0） |
| title | String | 是 | `"Spring Boot 进阶教程"` | 文章标题，最多 200 字符 |
| summary | String | 是 | `"本文深入讲解 Spring Boot 的高级特性..."` | 文章概述，最多 500 字符 |
| content | String | 是 | `"# Spring Boot 进阶\n\n深入探讨..."` | 文章内容（Markdown 格式） |
| isTop | Integer | 否 | `1` | 是否置顶：0-否，1-是（默认 0） |
| isDraft | Integer | 否 | `0` | 是否草稿：0-已发布，1-草稿（默认 0） |
| isDeleted | Integer | 否 | `0` | 是否删除：0-否，1-是（默认 0） |
| scategoryId | String | 否 | `"2,3,5"` | 子标签 ID 字符串，逗号分隔，最多 5 个 |
...
| data.isDeleted | Integer | 删除标记 |
| data.createdAt | DateTime | 创建时间 |
| data.updatedAt | DateTime | 更新时间（会自动更新为当前时间） |
...
```json
{
  ...
  "data": {
    ...
    "summary": "本文深入讲解 Spring Boot 的高级特性...",
    "content": "# Spring Boot 进阶\n\n深入探讨...",
    "isTop": 1,
    "isDraft": 0,
    "isDeleted": 0,
    "createdAt": "2026-04-15T10:00:00",
    "updatedAt": "2026-04-15T15:58:00",
    "username": "admin"
  }
}
```
...
- **接口名称**: 删除文章（软删除）
- **请求方法**: DELETE
- **请求路径**: `/api/article/delete/{id}`
- **功能描述**: 软删除指定文章，仅标记为已删除（需要登录权限）
...
- **接口名称**: 更新文章草稿状态
- **请求方法**: PUT
- **请求路径**: `/api/article/draft`
- **功能描述**: 更新文章的草稿状态（需要登录权限）
...
|--------|------|------|--------|------|
| articleId | Long | 是 | `1` | 文章 ID |
| isDraft | Integer | 是 | `1` | 草稿状态：0-已发布，1-草稿 |
...
| isDeleted | Integer | 否 | `0` | 删除状态：0-否，1-是（可选，默认不筛选） |
| pageNum | Integer | 否 | `1` | 页码，默认 1 |
| pageSize | Integer | 否 | `10` | 每页数量，默认 10 |
...
- **接口名称**: 物理删除文章
- **请求方法**: DELETE
- **请求路径**: `/api/article/permanent/{id}`
- **功能描述**: 彻底删除文章，从数据库中永久移除（需要登录权限）
...
- **接口名称**: 恢复文章
- **请求方法**: PUT
- **请求路径**: `/api/article/restore/{id}`
- **功能描述**: 恢复已软删除的文章，将 is_deleted 设为 0（需要登录权限）
